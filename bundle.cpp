...
void BundleAdjustment::adjustBundle(vector<Point3D>& pointCloud, vector<Matx34d>& cameraPoses, Mat intrinsic, vector<float>& distortion ,const vector<vector<Point2d>>& image2dFeatures) {
    ...
    using CameraVector = Matx<double,1,6>;
    ...
    double focal = (double)intrinsic.at<double>(0, 0);

    vector<Vec3d> points3d(pointCloud.size());

    for(int i = 0; i < pointCloud.size(); i++) {
        const Point3D& p = pointCloud[i];
        points3d[i] = Vec3d(p.pt.x, p.pt.y, p.pt.z);

        for(const pair<const int,int>& kv : p.idxImage) {
            //kv.first  = camera index
            //kv.second = 2d feature index

            Point2d p2d = image2dFeatures[kv.first][kv.second];

            //subtract center of projection, since the optimizer doesn't know what it is
            double cx = (double)intrinsic.at<double>(0, 2);
            double cy = (double)intrinsic.at<double>(1, 2);
            p2d.x -= cx;
            p2d.y -= cy;
    ...
    intrinsic.at<double>(0, 0) = (double)focal;
    intrinsic.at<double>(1, 1) = (double)focal;
    cout << "New Optimized K=\n" << intrinsic << endl;
